...
This is a standard Next.js App Router app (`vercel.json` + `next build`). The build runs `prisma generate`, `prisma db push`, and the optional public-episode seed. Following a show, **Check for new episodes**, and opening Library when the latest episode still needs a brief write that recap in-request. Production cron wakes daily at ~8am America/Chicago (`0 13 * * *` UTC; Hobby rejects weekday-only expressions), diffs each follow’s latest episode against the last-briefed ledger, and self-chains `/api/cron/continue` hops so STT + brief + TTS finish across many 300s invocations (Hobby plans cannot add extra crons). Vercel cron does not run on Preview — Check starts the same hop chain.
