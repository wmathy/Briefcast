...
- Follow, Library / show **Check**, Library auto-write, and daily production cron share **one** awaited generate pipeline (`refreshFollowedBriefs`). Candidates are ordered **newest `publishedAt` first across all followed shows**. Each invocation runs **one** 300s step on that prioritized episode (up to two 2MB STT chunks, or write the brief, or TTS), persists progress (`SttJob`, written brief before audio, `Follow.lastBriefedEpisodeId` ledger), then the server schedules the next hop (`POST /api/pipeline/continue`, skip RSS). A locked unfinished STT on a newer episode is not abandoned to start an older one. Hobby Vercel allows one daily cron (`15 8 * * *` UTC) and rejects weekday-only expressions at deploy time. Check is the same catch-up path and also starts the hop chain. Vercel cron does not run on Preview — use Check there.
