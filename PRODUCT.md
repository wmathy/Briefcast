...
- Auto-brief and the Library “waiting” count only include a **recent window**: the **newest** episode per followed show. Older archive rows stay on the show page and are not queued.
- Follow, Library / show **Check**, Library auto-write, daily production cron, and the optional GitHub 15-minute wake share **one** ACK-first generate pipeline. The HTTP handler returns **202 immediately**; `after()` / `runPipelineTurn` runs `refreshFollowedBriefs` with a 265s STT budget so leftover time can **await** the next hop’s 202 (never `void` a hop fetch, never abort at 8s). Candidates are the newest episode per follow; `pickFinishableNewest` prefers a public transcript, then in-progress STT, then shortest audio, and rotates off a fresh lock or 30-minute STT stall so a 3-hour JRE cannot starve Candace. Each invocation runs **one** 300s step (up to two 2MB STT chunks, or write the brief, or TTS), persists progress (`SttJob`, written brief before audio, `Follow.lastBriefedEpisodeId` ledger), then awaits `POST /api/pipeline/continue` (skip RSS). Pipeline hops authorize with `CRON_SECRET` or `AUTH_SECRET`. Hobby Vercel allows one daily cron (`15 8 * * *` UTC). Set GitHub secret `CRON_SECRET` so `.github/workflows/pipeline-continue.yml` can wake the same path every 15 minutes. Library still keeps a backup continue loop. Vercel cron does not run on Preview — use Check there.
...
- Source policy: a brief is published **only** from a verified **full episode transcript**. Sources, in order: stored or RSS `podcast:transcript`, official transcript pages (including NPR `/transcripts/` and `text.npr.org`), public captions, then **Grok Speech-to-Text of the entire audio file** in resumable ~2 MB chunks (Range requests, retries, persisted partial text). Completeness is checked against episode duration (at least ~100 words/minute of coverage, and STT audio coverage ≥ 85% when duration is known). Show notes are never the summarized body. If no complete transcript can be obtained, **nothing is published** — the episode and Library say “No full transcript yet”, and Check / follow / cron retry. Existing notes-only briefs are purged.
