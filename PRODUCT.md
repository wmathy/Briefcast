...
- Follow, Library / show **Check**, Library auto-write, and weekday production cron share **one** awaited generate pipeline (`refreshFollowedBriefs`). Each invocation runs **one** 300s step (one STT chunk, or write the brief, or TTS), persists progress (`SttJob`, written brief before audio, `Follow.lastBriefedEpisodeId` ledger), then the server schedules the next hop (`POST /api/cron/continue`, skip RSS) so a long episode finishes without Check/continue babysitting. Hobby Vercel allows one cron per day; the schedule is weekday ~8am America/Chicago (`0 13 * * 1-5` UTC). Weekend posts wait until the next weekday wake; Check is the same catch-up path and also starts the hop chain. Vercel cron does not run on Preview — use Check there.
