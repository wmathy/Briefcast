...
- Follow, Library / show **Check**, Library auto-write, and daily production cron share **one** awaited generate pipeline (`refreshFollowedBriefs`). Each invocation runs **one** 300s step (one STT chunk, or write the brief, or TTS), persists progress (`SttJob`, written brief before audio, `Follow.lastBriefedEpisodeId` ledger), then the server schedules the next hop (`POST /api/pipeline/continue`, skip RSS) so a long episode finishes without Check/continue babysitting. Hobby Vercel allows one daily cron (`15 8 * * *` UTC) and rejects weekday-only expressions at deploy time. Check is the same catch-up path and also starts the hop chain. Vercel cron does not run on Preview — use Check there.
