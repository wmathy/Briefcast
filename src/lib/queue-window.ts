import { orderAutoBriefQueue } from "@/lib/auto-brief-policy";
...
/** Rotate off a newest whose STT has not advanced — 30m, not 6h, so a dead hop chain cannot starve Candace. */
export const STT_STALL_MS = 30 * 60 * 1000;
...
export function isFreshlyLockedStt(item: FinishableWork, now = Date.now()): boolean {
  return item.sttStatus === "running" && sttLockIsFresh(item.sttLockedAt, now);
}

/**
 * One finishable newest, or nothing if the only candidate is locked.
 * Do not fall back to a fresh lock — that hop-spins to PIPELINE_MAX_HOPS and dies.
 */
export function selectWindowedAutoBriefIds(items: FinishableWork[], now = Date.now()): string[] {
  const finishable = pickFinishableNewest(items, now);
  if (finishable.length > 0) return finishable;
  const unlocked = items.filter((item) => !isFreshlyLockedStt(item, now));
  if (unlocked.length === 0) return [];
  const withSource = unlocked.filter((item) => item.hasSource !== false);
  const pool = withSource.length > 0 ? withSource : unlocked;
  return takeSingleNewestWork(orderAutoBriefQueue(pool));
}
...
export function pickFinishableNewest(items: FinishableWork[], now = Date.now()): string[] {
  ...
  const available = unbriefed.filter((item) => !isFreshlyLockedStt(item, now));
